...
#[tokio::test]
async fn test_object_store_not_registered_error() -> anyhow::Result<()> {
    // Test #3 from docs/new-tests.md: Object Store Not Registered
    // This test verifies that querying S3 paths without registering the object store
    // produces a helpful error message rather than panicking.

    // Skip test if Docker is not available
    if !is_docker_available().await {
        eprintln!("Skipping object store not registered test: Docker not available");
        return Err(anyhow::anyhow!("Test requires Docker"));
    }
    ...
    let bucket_name = "unregistered-bucket";
    s3_client
        .create_bucket()
        .bucket(bucket_name)
        .send()
    ...
    eprintln!("Bucket '{}' created", bucket_name);

    // Create temporary directory for catalog metadata
    let temp_dir = TempDir::new()?;
    let catalog_path = temp_dir.path().join("test_catalog.ducklake");
    let catalog_path_str = catalog_path.to_string_lossy().to_string();

    // Create catalog with S3 data using DuckDB
    eprintln!("Creating test catalog with S3 paths...");
    create_s3_test_catalog(
        &catalog_path_str,
        &minio_endpoint,
        bucket_name,
    ...
    .await?;
    eprintln!("Test catalog created with S3 references");

    // Create DataFusion session WITHOUT registering the S3 object store
    // This is the key difference - we intentionally omit object store registration
    let runtime = Arc::new(RuntimeEnvBuilder::new().build()?);
    // NOTE: NOT calling runtime.register_object_store() - this is intentional!
    ...
    // Attempt to query the table - this should fail with a helpful error
    eprintln!("Attempting query without registered object store...");
    let result = ctx
        .sql("SELECT * FROM ducklake.main.products ORDER BY id")
        .await;
    ...
    match result {
        Ok(df) => {
            // If SQL parsing succeeds, the error might occur during execution
            eprintln!("SQL parsing succeeded, attempting execution...");
            let exec_result = df.collect().await;

            match exec_result {
                Ok(_) => {
                    panic!("Query unexpectedly succeeded! Expected error about unregistered object store.");
                }
                Err(e) => {
                    let error_msg = e.to_string();
                    eprintln!("Got expected error during execution: {}", error_msg);

                    // Verify the error message is helpful
                    // DataFusion should mention something about object store not being found
                    let error_lower = error_msg.to_lowercase();
                    let has_helpful_context = error_lower.contains("object store")
                        || error_lower.contains("no suitable object store")
                        || error_lower.contains("not found")
                        || error_lower.contains("s3://");

                    assert!(
                        has_helpful_context,
                        "Error message should mention object store or S3 path. Got: {}",
                        error_msg
                    );

                    eprintln!("✓ Error message is reasonably helpful for users");
                    eprintln!("  Full error: {}", error_msg);
                }
            }
        }
        Err(e) => {
            // Error during SQL parsing/planning
            let error_msg = e.to_string();
            eprintln!("Got error during planning: {}", error_msg);
    ...
    eprintln!("Test passed: Object store not registered produces helpful error");
