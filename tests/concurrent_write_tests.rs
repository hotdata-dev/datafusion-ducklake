...
async fn test_concurrent_snapshot_creation() {
    ...
    assert_eq!(unique_count, snapshot_ids.len(), "Snapshot IDs should be unique");
}
...
async fn test_concurrent_writes_different_tables() {
    ...
    snapshot_ids.sort();
    let unique_count = snapshot_ids.windows(2).filter(|w| w[0] != w[1]).count() + 1;
    assert_eq!(unique_count, snapshot_ids.len(), "Each write should get unique snapshot ID");
}
...
async fn test_concurrent_writes_same_table_append() {
...

    {
        ...
        table_writer.write_table("main", "shared_table", &[batch]).unwrap();
    }
...
async fn test_write_session_cleanup_on_drop() {
    ...
    let file_path = {
        let table_writer = DuckLakeTableWriter::new(Arc::clone(&writer)).unwrap();
        let mut session = table_writer.begin_write("main", "dropped_table", &schema, true).unwrap();
        let batch = create_user_batch(&[1, 2, 3], &["a", "b", "c"]);
        session.write_batch(&batch).unwrap();
        session.file_path().to_path_buf()
    };
    assert!(!file_path.exists(), "Orphaned file should be deleted on drop");

    // Finished session should keep file
    let finished_path = {
        let table_writer = DuckLakeTableWriter::new(Arc::clone(&writer)).unwrap();
        let mut session = table_writer.begin_write("main", "finished_table", &schema, true).unwrap();
        let batch = create_user_batch(&[1, 2, 3], &["a", "b", "c"]);
        session.write_batch(&batch).unwrap();
...
async fn test_write_batch_schema_validation() {
    ...
    let schema = create_user_schema();
    let table_writer = DuckLakeTableWriter::new(Arc::clone(&writer)).unwrap();
    let mut session = table_writer.begin_write("main", "validation_test", &schema, true).unwrap();

    // Valid batch succeeds
    session.write_batch(&create_user_batch(&[1], &["valid"])).unwrap();
...
async fn test_transaction_atomicity() {
    ...
    let result = table_writer.write_table("main", "atomic_test", &[batch]).unwrap();
